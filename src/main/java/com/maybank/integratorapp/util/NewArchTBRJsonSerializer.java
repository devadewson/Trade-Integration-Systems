...
public class NewArchTBRJsonSerializer extends JsonSerializer<MsgBody> {
    @Override
    public void serialize(MsgBody wrapper, JsonGenerator gen, SerializerProvider serializers) throws IOException {
        ...
        for (Field field : MsgBody.class.getDeclaredFields()) {
            try {
                field.setAccessible(true); // Make private fields accessible
                String fieldName = field.getName();
                if(fieldName == ("TBRData")){
