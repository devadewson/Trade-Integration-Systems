package com.maybank.integratorapp.util.swiftconverter.converterfunction;

import com.maybank.integratorapp.util.swiftconverter.customexception.TransformationException;
import jakarta.annotation.PostConstruct;
import lombok.extern.slf4j.Slf4j;
import org.springframework.stereotype.Component;

import java.util.HashMap;
import java.util.Map;

@Component
@Slf4j
public class ExtractAddressFunction implements TransformationFunction {

    private final FunctionRegistry functionRegistry;

    public ExtractAddressFunction(FunctionRegistry functionRegistry) {
        this.functionRegistry = functionRegistry;
    }

    @PostConstruct
    public void init() {
        functionRegistry.registerFunction("extractAddress", this);
    }

    @Override
    public Object execute(String input) {
        Map<String, String> result = new HashMap<>();

        if (input == null) { // Minimum: 6 date + 3 currency
            log.warn("Invalid input for extractAddress: {}", input);
            // Create a structured object for the template
//            result.put("Ctry", "-");
//            result.put("AdrLine1", "-");
//            result.put("AdrLine2", "-");
            return result;
        }

        try {


            // Create a structured object for the template
            result.put("Ctry", "-");
            result.put("AdrLine1", "-");
            result.put("AdrLine2", "-");
            // You might also want to format the date for the MX message
            // result.put("Date", formatValueDate(valueDate));

            return result;

        } catch (Exception e) {
            log.error("Error splitting address from input: {}", input, e);
            throw new TransformationException("Failed to split address");
        }
    }
}
